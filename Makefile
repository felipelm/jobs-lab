...
docker-run-api:
	docker run --rm -p 8000:8000 -e DATABASE_URL="$(DOCKER_DATABASE_URL)" -e REDIS_URL="$(DOCKER_REDIS_URL)" -e OTEL_ENABLED="$(OTEL_ENABLED)" -e OTEL_EXPORTER_OTLP_ENDPOINT="$(OTEL_EXPORTER_OTLP_ENDPOINT)" $(API_IMAGE)
